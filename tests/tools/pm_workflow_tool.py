...
_PREFLIGHT_REQUIRED_ASSIGNEES = {"backend-specialist"}
...
def _run_worker_preflight(assignees: list[str]) -> dict[str, Any]:
    ...
    Quick chat is the source of truth. Gateway health must also be healthy.
    Auth snapshot is advisory only.
    """
    ...
    for assignee in required:
        ...
        worker_ok = bool(chat.get("ok")) and bool(gateway.get("ok"))
        if not worker_ok:
            all_ok = False
...
def _is_architect_task(task: dict[str, Any]) -> bool:
    return str(task.get("assignee") or "").strip() == "backend-specialist" and str(task.get("mode") or "").strip() == "architect"
...
def _normalize_tasks(raw_tasks: Any, *, require_pm_finalizer: bool = True) -> tuple[list[dict[str, Any]], str | None]:
    ...
    for idx, raw in enumerate(raw_tasks, start=1):
        ...
        if assignee == "backend-specialist" and not mode:
            return [], f"{key}: backend-specialist tasks require mode={sorted(_WORKER_MODES)}"
...
def _build_workflow_contract(plan: dict[str, Any]) -> dict[str, Any]:
    ...
    design_artifact = _explicit_design_artifact(plan) or artifacts.get("design") or ""
    ...
    design_concrete_paths = _extract_design_artifact_concrete_paths(
    ...
    implementation_paths = explicit_implementation_paths or design_concrete_paths or explicit_concrete_expected
...
def _export_approved_scope(
    ...
    scope = {
        ...
        "approval": {
            ...
            "approved_by": approved_by or plan.get("design_approved_by") or plan.get("approved_by"),
            "approved_at": time.strftime("%Y-%m-%dT%H:%M:%S%z", time.localtime(approved_at or int(time.time()))),
        },
...
def _validate_staged_plan_graph(tasks: list[dict[str, Any]], expected_deliverables: list[str]) -> list[str]:
    ...
    if not architect:
        errors.append("staged workflow requires at least one backend-specialist architect task")
    if not implementer:
        errors.append("staged workflow requires at least one backend-specialist implementer task after design approval")
    if not reviewer:
        errors.append("staged workflow requires at least one backend-specialist reviewer task after implementation")
    if not finalizers:
...
def _extract_design_artifact_concrete_paths(project_path: str, design_artifact: str) -> list[str]:
    ...
    return [path for path in extracted if _is_concrete_implementation_path(path)]
...
def _validate_workflow_contract_invariants(plan: dict[str, Any], contract: dict[str, Any]) -> list[str]:
    ...
    if has_implementer and design_extension_families and implementation_extension_families and design_extension_families.isdisjoint(implementation_extension_families):
        errors.append(
...
def pm_create_kanban_workflow(
    ...
    assignees = ["backend-specialist", "project_manager"]
...
def pm_execute_plan(plan_id: str, approved_by: str = "user") -> str:
    ...
    if not design_tasks:
        return _error("plan must include at least one backend-specialist architect task before execution", plan_id=plan_id)
...
_TASK_ITEM_SCHEMA = {
    ...
    "properties": {
        ...
        "title": {"type": "string"},
        "body": {"type": "string"},
        "assignee": {"type": "string", "enum": ["backend-specialist", "project_manager"]},
        "mode": {"type": "string", "enum": ["architect", "implementer", "debugger", "reviewer"], "description": "Required for backend-specialist tasks; omit for project_manager tasks."},
