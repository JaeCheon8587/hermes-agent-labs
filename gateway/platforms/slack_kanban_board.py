...
def _connect_board(board: str | None):
    """Open a Kanban board connection and always close it.

    sqlite3.Connection as a context manager only commits/rolls back; it does
    not close file descriptors. Slack gateway code is long-lived, so every
    short-lived board operation must close explicitly to avoid retaining
    deleted WAL/SHM sidecars.
    """
