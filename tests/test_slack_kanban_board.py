...
from gateway.platforms import slack_kanban_board as board
...
def test_load_tasks_closes_connection_and_does_not_force_init(monkeypatch):
    conn = _TrackingConn()
    calls = {"init": 0}

    def fail_init(*args, **kwargs):
        calls["init"] += 1
        raise AssertionError("read path must not force kanban init_db")

    monkeypatch.setattr(board.kb, "init_db", fail_init)
    monkeypatch.setattr(board.kb, "connect", lambda **kwargs: conn)
    monkeypatch.setattr(board.kb, "list_tasks", lambda *_args, **_kwargs: [])

    assert board._load_tasks(BoardFilters(board="launch")) == []
    assert conn.closed is True
    assert calls["init"] == 0


def test_load_tasks_closes_connection_when_rendering_fails(monkeypatch):
    conn = _TrackingConn()

    monkeypatch.setattr(board.kb, "connect", lambda **kwargs: conn)

    def raise_from_list(*_args, **_kwargs):
        raise RuntimeError("boom")

    monkeypatch.setattr(board.kb, "list_tasks", raise_from_list)

    try:
        board._load_tasks(BoardFilters(board="launch"))
    except RuntimeError as exc:
        assert str(exc) == "boom"
    else:
        raise AssertionError("expected list_tasks failure")
